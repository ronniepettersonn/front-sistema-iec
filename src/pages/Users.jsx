...
export function Users() {
  ...
  async function handleGetMembers() {
    const response = await api.get('/users').finally(() => setIsLoading(false))
  ...
  async function handleDelete(id, email) {
    ...
    await supabase.storage.from('profile-member').remove([`${formartEmail(email)}`])
        ...
        <div className='flex flex-col flex-1 p-8'>
          ...
          <div className='flex pb-8 border-b border-slate-700 justify-between'>
            <div className="flex items-center justify-between">
              <h1 className="text-2xl font-bold leading-none text-gray-900 dark:text-white">Lista de Usuários</h1>
            </div>
            <Link to='/create/user'>
                          ...
                          return (
                            <li className={`py-3 sm:py-4`} key={member.id}>
                              <div className="flex items-center space-x-4">
                                ...
                                <img className="w-14 h-14 object-cover rounded-full" src={`https://ixxycvckmqbqsngzgpbl.supabase.co/storage/v1/object/public/profile-member/${formartEmail(member.email)}`} width="48" height="48" alt="User" />
                                ...
                                <div className="flex md:flex-1 justify-end gap-4">
                                                ...
                                                <Dialog.Portal>
                                                  ...
                                                  <Dialog.Content className="bg-gray-700 z-40 fixed top-[calc(50%-119px)] left-[calc(50%-200px)] w-[400px] rounded-lg p-6">
                                                    ...
                                                    <Dialog.Description className=' text-center'>
                                                      ...
                                                      <Dialog.Close asChild>
                                                        <button onClick={() => handleDelete(member.id, member.email)} data-modal-hide="popup-modal" type="button" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                                                          Sim, excluir
                                                        </button>
                                                      </Dialog.Close>
                                                      <Dialog.Close>
                                  ...
                                  <Link to={`/users/update/${member.id}`}>
